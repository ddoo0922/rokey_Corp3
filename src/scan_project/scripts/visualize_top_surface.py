...
def save_height_map_image():
    ...
    plt.title("Top Surface Height Map")
    plt.xlabel("X pixel")
    plt.ylabel("Y pixel")
    plt.tight_layout()

    save_path = os.path.join(IMAGE_DIR, "top_height_map.png")
    plt.savefig(save_path, dpi=200)
    plt.close()

    print(f"[OK] saved height map image: {save_path}")


def save_3d_surface_image():
    ply_path = os.path.join(MESH_DIR, "top_surface_points.ply")
    ...
    ax.set_title("Top Surface 3D Shape")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_zlabel("Z Height (m)")

    save_path = os.path.join(IMAGE_DIR, "top_surface_3d_shape.png")
    plt.tight_layout()
    plt.savefig(save_path, dpi=200)
    plt.close()

    print(f"[OK] saved 3D surface image: {save_path}")


def main():
    save_height_map_image()
    save_3d_surface_image()
