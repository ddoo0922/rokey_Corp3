...
def main():
    ...
    world.scene.add(
        VisualCylinder(
            ...
            name="robot_pedestal",
            position=np.array([-0.8, -0.7, 0.1]), # 0.2 높이의 중간 지점
            radius=0.12,
            height=0.2,
    ...
    while simulation_app.is_running():
        ...
        if world.is_playing() and current_target_idx < len(points):
            ...
            controller._motion_policy.set_robot_base_pose(
                robot_position=np.array([-0.8, -0.7, 0.2]),
            ...
            # 사용자님이 원하시는 "아까 잘 조립되었던" 상태(assemble_robot.py Z=-0.015)에서 
            # 플랜지 끝단과 패드 바닥의 실제 거리는 약 0.8cm(0.008)입니다.
            # 로봇 팔(link_6)을 타겟에서 딱 0.8cm만 띄우면 패드가 표면에 완벽하게 맞닿습니다.
