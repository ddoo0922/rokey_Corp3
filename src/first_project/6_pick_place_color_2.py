...
rmpflow_dir = os.path.expanduser(
    "~/cobot3_ws/isaacpjt/M0609/rmpflow"
)
...
RGB_PUBLISH_FREQUENCY = 15

# 실제 Wrist Camera Prim 경로를 알고 있으면 문자열로 지정하세요.
# None이면 Stage 내부 Camera Prim 중 wrist/hand/link_6/camera 키워드 순으로 자동 탐색합니다.
...
def find_camera_prim_path(preferred_path=None):
    stage = omni.usd.get_context().get_stage()

    if preferred_path:
        prim = stage.GetPrimAtPath(preferred_path)
        if prim.IsValid() and prim.IsA(UsdGeom.Camera):
            print(f"[Camera] 지정 경로 사용: {preferred_path}")
            return preferred_path
        print(f"[WARN] 지정한 Camera Prim 경로가 유효하지 않습니다: {preferred_path}")
    ...
    for keyword in ("wrist", "hand", "link_6", "camera"):
        for path in camera_paths:
            if keyword in path.lower():
                print(f"[Camera] 자동 선택: {path}")
                return path

    print(f"[Camera] 첫 번째 Camera Prim 선택: {camera_paths[0]}")
    return camera_paths[0]


def setup_ros2_rgb_publisher(camera_prim_path):
    ...
    rgb_render_var = (
        ...
        .convert_sensor_type_to_rendervar(sd.SensorType.Rgb.name)
    )

    writer_name = rgb_render_var + "ROS2PublishImage"
    rgb_writer = rep.writers.get(writer_name)

    rgb_writer.initialize(
        frameId=CAMERA_FRAME_ID,
        nodeNamespace="",
        queueSize=1,
        topicName=RGB_TOPIC_NAME,
    )
    rgb_writer.attach([render_product_path])

    step_size = max(1, int(60 / RGB_PUBLISH_FREQUENCY))
    gate_path = omni.syntheticdata.SyntheticData._get_node_path(
        rgb_render_var + "IsaacSimulationGate",
        render_product_path,
    )
    og.Controller.attribute(gate_path + ".inputs:step").set(step_size)

    print("\n" + "=" * 60)
    print("[ROS2 RGB Publisher 실행]")
    print(f"  Camera     : {camera_prim_path}")
    print(f"  Topic      : /{RGB_TOPIC_NAME}")
    print("  Type       : sensor_msgs/msg/Image")
    print(f"  Resolution : {CAMERA_RESOLUTION[0]} x {CAMERA_RESOLUTION[1]}")
    print(f"  Frequency  : 약 {RGB_PUBLISH_FREQUENCY} Hz")
    print("=" * 60)

    return render_product, rgb_writer
...
class ColorSubscriberNode(Node):
    def __init__(self):
        ...
        print("[ROS2] /color_id subscriber 생성 완료")
        self.get_logger().info(
            "토픽 대기 중... ros2 topic pub /color_id std_msgs/msg/Int32 \"{data: 1}\" --rate 1"
        )
    ...
    def _cb(self, msg):
        ...
        print("\n" + "=" * 50)
        print(f"[ROS2 수신 성공] /color_id = {self.color_id}")
        print("=" * 50 + "\n")

        self.get_logger().info(
            f"수신: color_id={self.color_id} (1=파랑, 2=초록)"
        )
...
def main():
    ...
    camera_path = find_camera_prim_path(CAMERA_PRIM_PATH)
    if camera_path is None:
        print("[WARN] Camera Prim을 찾지 못해 /rgb 발행을 시작하지 못했습니다.")
        print("[WARN] Pick & Place 시뮬레이션은 계속 실행합니다.")
    else:
        try:
            render_product, rgb_writer = setup_ros2_rgb_publisher(camera_path)
        except Exception as error:
            print(f"[WARN] /rgb Publisher 생성 실패: {error}")
            print("[WARN] Pick & Place 시뮬레이션은 계속 실행합니다.")
    ...
    if rgb_writer is not None:
        try:
            ...
            print(f"[WARN] RGB Writer detach 실패: {error}")
