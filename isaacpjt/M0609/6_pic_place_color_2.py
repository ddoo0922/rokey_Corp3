...
# 정확한 Wrist Camera 경로를 알고 있으면 여기에 입력
# 예: "/World/m0609/link_6/wrist_camera"
# None이면 Stage 안의 Camera Prim을 자동 검색
...
def setup_ros2_rgb_publisher(camera_prim_path):
    ...
    rgb_writer = rep.writers.get("ROS2PublishImage")
...
def main():
    ...
    # Wrist Camera → ROS2 /rgb 이미지 발행
    camera_path = find_camera_prim_path(CAMERA_PRIM_PATH)
    if camera_path is None:
        raise RuntimeError(
            "Stage에서 Camera Prim을 찾지 못했습니다. "
            "CAMERA_PRIM_PATH를 실제 Wrist Camera 경로로 지정하세요."
        )
